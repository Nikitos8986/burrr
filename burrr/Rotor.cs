...
namespace burrr
{
    public partial class Rotor : Form
    {
        ...
        private void label5_Click(object sender, EventArgs e)
        {
            ...
            obs.Upd("Роторные буровые установки");
            obs.ShowDialog();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (Мощность < 180 || Мощность > 280)
            {
                MessageBox.Show("Мощность должна быть в диапазоне от 180 до 280 кВт.");
                return;
            }
            double Глубина = double.Parse(textBox1.Text);
            if (Глубина < 30 || Глубина > 76)
            {
                MessageBox.Show("Глубина должна быть в диапазоне от 30 до 76 метров.");
                return;
            }
            double Масса = double.Parse(textBox3.Text);
            if (Масса < 8000 || Масса > 10000)
            {
                MessageBox.Show("Масса должна быть в диапазоне от 8000 до 10000 кг.");
                return;
            }
            ...
            string quere = $"select * from Роторные";
            SqlCommand cmd = new SqlCommand(quere, con);
            adapter.SelectCommand = cmd;
            adapter.Fill(Table);
            con.Open();
            SqlCommand insertCommand = new SqlCommand($"insert into Роторные(Мощность, Глубина_бурения, Масса, Цена) values ('{moch}','{glub}','{mass}','{stoim}')", con);
