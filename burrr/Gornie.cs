...
namespace burrr
{
    public partial class Gornie : Form
    {
        ...
        private void label5_Click(object sender, EventArgs e)
        {
            ...
            obs.Upd("Горные буровые установки");
            obs.ShowDialog();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (Мощность < 400 || Мощность > 500)
            {
                MessageBox.Show("Мощность должна быть в диапазоне от 400 до 500 кВт.");
                return;
            }
            double Глубина = double.Parse(textBox1.Text);
            if (Глубина < 10 || Глубина > 30)
            {
                MessageBox.Show("Глубина должна быть в диапазоне от 10 до 30 метров.");
                return;
            }
            double Масса = double.Parse(textBox3.Text);
            if (Масса < 8000 || Масса > 11000)
            {
                MessageBox.Show("Масса должна быть в диапазоне от 8000 до 11000 кг.");
                return;
            }
            ...
            string quere = $"select * from Горные";
            SqlCommand cmd = new SqlCommand(quere, con);
            adapter.SelectCommand = cmd;
            adapter.Fill(Table);
            con.Open();
            SqlCommand insertCommand = new SqlCommand($"insert into Горные(Мощность, Глубина_бурения, Масса, Цена) values ('{moch}','{glub}','{mass}','{stoim}')", con);
